...
public class DiningController implements Controller<DiningHall> {
    ...
    public DiningHall createNewItem(JSONObject diningHall, Context context) throws Exception {
            ...
            public void run() {
                try {
                    for (int i = 0; i < array.length(); i++) {
                        ...
                        diningHalls.add(createNewItem(diningHall, context));
                    }
                    ((Activity) context).runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
    ...
    public void setItem(@NonNull final Context context, final JSONObject obj) {
        try {
            setCurrentDiningHall(createNewItem(obj, context));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
