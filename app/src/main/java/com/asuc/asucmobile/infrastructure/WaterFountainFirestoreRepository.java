...
public class WaterFountainFirestoreRepository implements Repository<CategoryLoc> {

    public static final String TAG = "WaterFountainFirestore";

    private CategoryLocTransformer mTransformer;
    private CollectionReference mRef;

    public WaterFountainFirestoreRepository(FirebaseFirestore firestore) {
        Log.d(TAG, "Initialized");
        mTransformer = new CategoryLocTransformer();
        mRef = firestore.collection(FirebaseCollectionNames.WATER_FOUNTAIN);
    }
    ...
    public List<CategoryLoc> scanAll(final List<CategoryLoc> list, final RepositoryCallback<CategoryLoc> callback) {
        ...
        mRef.get().addOnFailureListener(new OnFailureListener() {
            ...
            public void onFailure(@NonNull Exception e) {
                Log.d(TAG, "Unable to retrieve water fountain data");
                callback.onFailure();
            }
        });
        return list;
